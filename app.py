...
# 버튼이 하는 일(재수집 요청 + 캐시 비우기 + store_only 안내)은
# views/_ui.py에 한 벌만 둡니다. 예전에는 이 블록이 화면 네 곳에 복사돼
# 있었고, st.cache_data.clear()만 부르던 버그를 고칠 때 실제로 일부만
# 고쳐지는 일이 있었습니다.
refresh_button("데이터 수동 새로고침 🚀", container=st.sidebar)
