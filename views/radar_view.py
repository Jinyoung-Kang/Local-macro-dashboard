...
def render_radar_view():
    """
    외국인/기관 수급 레이더(코스피) 화면을 그립니다.

    파라미터:
        없음. 조회 조건(시장·투자주체·방향·기간)은 화면 위젯에서
        사용자가 고릅니다.
    ...
    주의사항:
        - 조회 조건 조합이 많아 **수집기가 미리 받아 두는 것은 기본
          조합 몇 개뿐**입니다(collector.py의 _task_radar_rankings).
          기본값에서 벗어난 조합을 고르면 화면이 직접 수집하므로
          느립니다. 이건 고장이 아니라 의도된 절충입니다.
        - Naver·Daum은 과거 날짜 조회를 지원하지 않습니다. 과거 날짜를
          고르면 쓸 수 있는 소스가 크게 줄어, 우리가 쌓아 둔 누적
          이력으로 떨어질 수 있습니다.
    """
