...
def render_ai_report_view():
    ...
    주의사항:
        - AI에게 넘기는 컨텍스트는 **저장본에서 만들어집니다.** 저장본이
          오래됐으면 AI도 오래된 값을 근거로 결론을 씁니다. 리포트에
          수집 시각이 함께 나오는지 확인하세요.
        - ^MOVE 같은 추정치 지표는 요약문에 "추정치" 표시가 함께
          들어가야 합니다. 그 표시가 빠지면 AI가 공식 지표처럼 해석해
          잘못된 임계치 판단(예: MOVE 140 이상 = 채권 발작)을 내립니다.
        - 외부 LLM API를 호출하므로 느리고, 호출 비용이 발생할 수
          있습니다.
    """
