...
대시보드가 가진 데이터를 한 덩어리 텍스트로 만듭니다. AI 리포트의 입력이자
화면의 `전체 대시보드 원본 데이터 보기`가 보여 주는 것과 같은 글입니다.

[수치만 주면 모델이 방향을 뒤집습니다]
이 파일의 절반은 숫자가 아니라 **숫자를 읽는 법**입니다. 실제로 겪은 일들:

- 백분위 3.5%인 하이일드 스프레드를 "위험회피 신호"로 읽음
  → §2에 "다섯 지표 모두 높을수록 위험"과 국면 대응을 못 박음
- 경보선을 현재값보다 안전한 쪽에 그음(NFCI < -1.0)
  → §1·§10에 지표마다 `위험 방향` 한 줄을 붙임
- Daum 포털 집계를 "KRX 공식 확정치"라고 씀
  → §6의 소제목마다 출처를 따로 적음
- 등락률로 역산한 전일 종가를 공식 종가처럼 인용
  → `⚠️ 등락률 역산치(공식 종가 아님)`를 값 옆에 붙임

새 지표를 넣을 때도 값과 함께 **어느 쪽이 위험인지** 적어 주세요.
적지 않으면 모델은 절반의 확률로 반대로 읽습니다.
"""
import logging
...
def _append_macro_section(lines: list[str], macro_res):
    ...
    if r10_curr is not None and r2_curr is not None:
        ...
        lines.extend([
            ...
            # [정확성] §10의 심화 지표에는 위험 방향을 적어 두었는데 여기만
            # 빠져 있었습니다. 그래서 "10Y-2Y > 0.50%"을 경보선으로 잡은
            # 리포트가 나왔습니다(2026-09-16 00:08). 그 방향은 곡선이 더
            # 가팔라지는 쪽이라 침체 위험이 **줄어드는** 쪽입니다.
            "  · 위험 방향: 값이 **낮을수록**(0 아래로 역전될수록) 침체"
            " 위험이 큽니다. 경보선은 현재값보다 **낮은** 쪽에 그으십시오.",
        ])
