...
매크로 카드(환율·국채·원자재·주가지수)의 수집과 포맷.

[전일 대비를 계산할 때 조심할 것]
분봉으로 받은 종목의 `iloc[-2]`는 **1분 전 봉**이지 전일 종가가 아닙니다.
예전에 그것을 전일 종가로 써서, 코스피가 실제 -3.26%인 날 화면에
+0.09%로 떴습니다. 지금은 분봉이면 항상 일봉에서 직전 거래일 종가를
가져오고, 그마저 없으면 `전일 대비 미제공`으로 둡니다 —
0.00%로 위장하지 않습니다.

스크래핑으로 주입되는 항목(야간선물·해외지수 선물)은 전일 종가가
등락률 역산치일 수 있습니다. 그 경우 `prev_source`에 표시가 붙고
화면과 AI 스냅샷이 함께 밝힙니다.
"""
import io
import logging
