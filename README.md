...
셋 다 네트워크를 쓰지 않으므로 언제든 돌 수 있습니다 (현재 143건).
...
| 수급 레이더에서 `PyKrx/KRX` 빨간 카드 | PyKrx는 마지막 폴백입니다. KIS/Daum/Naver가 녹색이면 당일 조회는 정상이고, **과거 날짜 조회만** 영향을 받습니다. `pip install --upgrade pykrx` 로 갱신해 보세요 |
