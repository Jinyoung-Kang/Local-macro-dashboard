...
| 🇰🇷 국내 파생 & 투기세력 (KRX) | KOSPI200 선물 OI·베이시스·한국판 COT |
...
> **KRX 금액(억원) 기준**
> Daum API가 `type=PRICE` 요청을 무시하고 계약수를 반환하는 경우가 있습니다.
> 그대로 억 원으로 환산하면 전부 0이 되므로, 값의 크기로 판별해 계약수
> 기준으로 되돌리고 화면에 그 사유를 표시합니다.
...
둘 다 네트워크를 쓰지 않으므로 언제든 돌 수 있습니다 (현재 102건).
