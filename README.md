...
| AI 엔진이 `HTTP 410` / `서비스 종료` | 제공자가 그 모델을 내렸습니다. 대체 모델로 바꿔야 합니다. 2026-09-14 실측 기준 `openai/gpt-oss-120b`(09-03 종료)와 `meta/llama-3.3-70b-instruct`(08-26 종료)가 여기 해당합니다 |
| Cerebras가 `HTTP 404` | 모델 ID가 다르거나 **계정에 그 모델 권한이 없습니다**. Cerebras 콘솔에서 사용 가능한 ID를 확인해 `AI_MODEL_REGISTRY`의 `model` 값을 고치세요 |
| AI 리포트가 120초에서 `ReadTimeout` | 해결됐습니다. 비스트리밍이라 생성이 전부 끝날 때까지 한 번의 read 안에서 기다려야 했습니다. 지금은 스트리밍이라 토큰이 흐르는 동안 끊기지 않습니다 |
| AI 리포트가 `✂️ 중단` 표시와 함께 짧게 끝남 | 전체 제한 시간(`OVERALL_DEADLINE`, 기본 420초)에 걸렸습니다. 받은 부분까지는 보존됩니다. 입력을 줄이거나 더 빠른 엔진을 쓰세요 |
| COT 상세를 켜면 리포트가 느림 | 정상입니다. 입력이 커지면 첫 응답까지의 처리 시간이 늘어납니다. 화면이 입력 토큰 수를 표시하니 참고하세요 |
| 매크로 카드의 등락률이 실제와 다름 | 해결됐습니다. 분봉의 `iloc[-2]`(=1분 전 봉)를 전일 종가로 쓰고 있었습니다. 이제 분봉이면 항상 일봉에서 직전 거래일 종가를 가져옵니다 |
| AI 리포트에 `⚠️ 신뢰도가 높음인데...` 경고 | 모델이 본문에서는 상충을 말하면서 신뢰도를 '높음'으로 쓴 경우입니다. 결론을 그대로 받아들이기 전에 본문의 상충 부분을 확인하세요 |
| AI 리포트가 총평만 나오고 끝남 | 토큰 상한에 걸린 것입니다. 이제 `잘림` 표시와 경고가 함께 뜹니다. gpt-oss 계열은 사고 과정도 같은 예산을 깎아 쓰므로, 입력을 줄이거나 더 큰 모델을 쓰세요 |
| 수급 레이더 KIS 진단이 `데이터가 비어 있음` | 인증은 통과했고 돌려줄 행이 없는 상태입니다. **장중인데도** 이 메시지가 나오면 TR 파라미터나 계정의 시세 이용 권한을 확인하세요 |
| 수집 로그에 `No runtime found` 도배 | 해결됐습니다. 이 스크립트가 Streamlit 런타임 밖에서 도는 것은 정상이며, 해당 안내만 골라 거릅니다 |
| 수집 로그의 `KRX 로그인 실패: KRX_ID...` | 서드파티 `pykrx` 패키지 내부의 print라 우리 코드에서 끄지 않습니다. 동작에는 영향이 없습니다 |
