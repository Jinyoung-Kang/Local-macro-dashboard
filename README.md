...
| 📡 외국인/기관 수급 레이더 (코스피) | 코스피 투자자별 순매수 상위 · KIS/LS/Daum/Naver 다단 폴백 |
| 🗄️ 데이터 저장소 상태 | 수집 현황·신선도·실패 원인·누적 이력 |
| 🤖 AI 종합 데이터 분석 & 결론 리포트 | 수집 데이터 기반 AI 리포트 · 유형 3종 · 엔진 점검 · 결과 자동 점검(§7) |
...
수집된 데이터를 텍스트 스냅샷으로 만들어 AI 엔진에 보내고, 돌아온 글을
**구조화해 그립니다.** 리포트 유형 3종은 각각 다른 system prompt·판단
어휘·토큰 예산을 씁니다.

| 유형 | 판단 어휘 | 배너 라벨 |
|---|---|---|
| 종합 거시경제 & 수급 전략 | 위험선호 / 중립 / 위험회피 | 시장 국면 |
| 외국인/기관 수급 집중 분석 | 외국인 주도 매수 / 기관 주도 매수 / 혼조 / 동반 매도 | 수급 주도권 |
| 금리 및 유동성 리스크 점검 | 낮음 / 보통 / 높음 / 경계 | 리스크 수준 |

### 왜 자동 점검이 필요한가

프롬프트로 규칙을 정해도 모델이 지키지 않는 일이 실제로 반복됐습니다.
그래서 **화면이 결과를 다시 검사**합니다. 아래는 모두 실제 리포트에서
관찰된 결함이고, 각 검사는 그 사례를 고정한 회귀 테스트를 가집니다.

| 화면 표시 | 무엇을 잡는가 |
|---|---|
| 🧠 모델의 사고 과정 | 리포트 대신 생각 과정이 출력된 경우. **배너를 그리지 않습니다** — 그 안의 총평은 검토 중이던 초안입니다 |
| ✂️ 잘림 | `finish_reason == "length"`. 토큰 상한에 걸려 문장 중간에 끊긴 것 |
| ⚠️ 신뢰도 모순 | 본문은 상충을 말하는데 신뢰도가 '높음' |
| ⚠️ 판단-전략 모순 | 판단은 위험선호인데 대응 전략의 주식 항목이 모두 '축소' |
| 🚫 지어낸 보유 비중 | "현재 비중 30% → 20%" — 대시보드는 보유 내역을 수집하지 않습니다 |
| 📏 부등호 없는 조건 | 발생 조건 칸에 임계값 없이 현재값만 적힌 경우 |
| 🔤 어휘를 벗어난 판단 | 다른 유형의 판단 어휘를 가져다 쓴 경우 |
| 📝 프롬프트 지시문 유출 | 시스템 프롬프트 문장이 본문에 그대로 섞인 경우 |
| 📝 한자 혼입 | 한국어 리포트에 한자가 섞인 경우 |
| 🌐 번역되지 않은 총평 | 본문은 한국어인데 총평 항목만 영어인 경우 |
| 🐢 큰 입력 | 실제 Context가 6,000토큰을 넘고 엔진이 짧은 모델인 경우 |

### 문맥이 모델에게 알려 주는 것

수치만 주면 모델이 방향을 뒤집습니다. 그래서 스냅샷이 **해석의 방향까지**
함께 적습니다.

- §1 장단기 금리차 · §10 심화 지표: 지표마다 `위험 방향` 한 줄
- §2 신용·변동성: 다섯 지표가 모두 "높을수록 위험"이며 그것이 어느 국면인지
- §6: 소제목마다 출처 (KRX 공식 API / Daum 포털 집계)
- 역산한 전일 종가에는 `⚠️ 등락률 역산치(공식 종가 아님)`

### 엔진

`🩺 엔진 점검`으로 **내 키로 실제 응답하는 모델**을 확인할 수 있습니다.
모델 ID는 제공자가 예고 없이 내리기 때문에(HTTP 410), 목록을 믿지 말고
점검 결과를 믿으세요. `⚡ 자동 탐색`은 실측 응답 속도 순으로 실패 시
다음 엔진으로 넘깁니다.
...
| `tests/test_regressions.py` | 115 | 과거에 실제로 앱을 망가뜨렸던 버그들을 고정 (AI 리포트 품질 검사 포함) |
...
(**현재 265 passed · 3 skipped**. skip 3건은 Chromium 미설치 환경 기준입니다).
...
- 수급 폴백이 주말을 건너뛸 것
- AI 리포트 본문을 `res.get("response", ...)`로 꺼내지 않을 것
  (실패한 결과에도 `response`가 빈 문자열로 있어 화면이 조용히 비어집니다)
- 리포트 렌더링이 생성 버튼 블록 **밖**에서 일어날 것
  (안에서 그리면 자동 새로고침 때마다 리포트가 사라집니다)
- 모델 출력이 `unsafe_allow_html`에 들어가기 전에 `html.escape()`를 거칠 것
- 프롬프트의 굵은 지시문이 명령형으로 끝날 것
  (평서문이면 모델이 출력 한 줄로 착각해 그대로 베껴 씁니다)
- 판단 어휘가 화면의 색 매핑(`_JUDGEMENT_TONE`)에 모두 있을 것
...
services/
  ...
  ai_service.py         AI 엔진 라우팅(NVIDIA / Cloudflare / Cerebras) ·
                        SSE 스트리밍 · 엔진 점검 · 리포트 파싱 · 품질 검사
  prompts.py            리포트 프롬프트 라이브러리 (유형 3종 · 블록 조립)
  dashboard_snapshot_service.py  전체 원본 데이터 텍스트 생성
                        (출처·위험 방향까지 함께 적어 보냅니다)
...
| 리포트 제목이 `... 분석 분석 리포트` | 해결됐습니다. 유형 이름이 이미 "분석"으로 끝나는데 제목에 한 번 더 붙였습니다 |
| 총평의 `낮음`이 판단인지 신뢰도인지 헷갈림 | 해결됐습니다. 금리 리포트는 판단 어휘가 신뢰도와 겹칩니다. 이제 판단 앞에 **리스크 수준**이라는 라벨이 붙고, 방향이 아니므로 화살표 대신 ●를 씁니다 |
| AI 리포트에 `⚠️ 총평은 위험선호인데...` 경고 | 총평의 판단과 대응 전략의 주식 비중 방향이 반대입니다. 둘 중 하나가 틀렸다는 뜻이니 본문에서 실제 결론을 확인하세요 |
| 리포트가 Daum 수치를 `KRX 공식 확정치`라고 씀 | 해결됐습니다. Context의 6번 섹션 머리글이 하나뿐이라 두 출처가 섞여 읽혔습니다. 이제 소제목마다 출처를 답니다 |
| 리포트 표가 전부 `데이터 없음` | 실제로 제공된 데이터를 모델이 못 찾은 경우가 많습니다. 프롬프트에 "모든 섹션을 확인하고 적으라"는 규칙을 넣었습니다. 그래도 반복되면 해당 소스의 수집이 실패한 것인지 `--status`로 확인하세요 |
| 총평만 영어로 나옴 | 본문이 한국어면 자동 번역을 건너뛰기 때문입니다. 배너 아래에 🌐 안내가 뜹니다. 내용은 유효하니 그대로 읽으셔도 됩니다 |
| 리포트에 `🚫 포트폴리오 비중` 경고 | 모델이 "현재 비중 30 % → 20 %"처럼 보유 비중을 지어낸 것입니다. **이 대시보드는 보유 내역을 수집하지 않습니다.** 방향(확대/유지/축소)만 참고하세요 |
| 리포트가 없는 기관 이름을 붙임(예: `KIS 수급`) | 프롬프트에 "문맥에 없는 기관 이름을 붙이지 말 것"을 넣었습니다. 그래도 보이면 원본 데이터 스냅샷의 해당 섹션 출처와 대조하세요 |
| 리스크·경보 조건이 이미 충족돼 있음 | 조건은 아직 일어나지 않은 값이어야 합니다. 프롬프트에 규칙을 넣었지만 모델이 어길 수 있으니, 현재값과 비교해 보세요 |
| 리포트 본문에 지시문이 섞여 나옴 | 모델이 프롬프트를 출력의 일부로 착각한 것입니다. 배너 아래 📝 안내가 뜨며, 그 줄은 분석 내용이 아니니 무시하세요 |
| 리포트에 한자가 섞임(예: `突破`) | 모델이 한국어 외 문자를 흘린 것입니다. 띄어쓰기도 함께 깨져 있을 수 있으니 다른 엔진으로 다시 생성해 보세요 |
| 리포트가 스프레드 방향을 거꾸로 해석 | 예: "HY OAS 2.65%가 위험회피를 지시한다". 스프레드·변동성·금융스트레스는 **값이 높을수록 위험**입니다. 이제 원본 데이터에 위험 방향을 함께 적어 보냅니다 |
| 경보선이 현재값보다 안전한 쪽에 있음 | 예: 현재 NFCI -0.564인데 경보선 -1.0. 영원히 울리지 않습니다. 프롬프트에 "경보선은 현재값보다 위험한 쪽" 규칙을 넣었습니다 |
| 리포트가 지표군에 반대 국면 라벨을 붙임 | 예: "변동성은 위험선호를 시사". 원본 데이터 §2에 국면 대응(변동성 높음=위험회피, 스프레드 낮음=위험선호)을 명시했습니다 |
| `10Y-3M이 10Y-2Y보다 크다`를 이상 신호로 해석 | 3M < 2Y이므로 정상 곡선에서 당연한 관계입니다. §10에 참고 문구를 넣었고, 프롬프트에 "크기 비교는 상충이 아니다"를 못 박았습니다 |
| 반증 조건이 오히려 판단을 뒷받침함 | 프롬프트에 "반증 조건은 판단을 약화시키는 쪽" 규칙을 넣었습니다. 그래도 보이면 그 항목은 반증이 아니니 무시하세요 |
| 리포트에 `📏 발생 조건에 부등호가 없음` 경고 | 모델이 조건 칸에 현재값만 적은 것입니다(예: "VIX 17.47"). 언제 발생하는지 알 수 없으니 그 행은 경보로 쓰지 마세요 |
| `10Y-2Y > 0.50%`처럼 경보 방향이 반대 | 장단기 금리차는 **낮아질수록** 위험합니다. 원본 데이터 §1에 위험 방향을 함께 적도록 고쳤습니다 |
| 내려받은 .md에 `---`가 두 줄 연달아 나옴 | 해결됐습니다. 모델이 본문 끝에 붙인 구분선을 떼고 하나만 답니다 |
| 리포트에 `🧠 모델의 사고 과정` 오류 | 모델이 리포트 대신 생각한 과정을 출력한 것입니다. 그 안의 총평은 검토 중이던 **초안**이므로 배너를 그리지 않습니다. 다른 엔진으로 다시 생성하거나 입력을 줄이세요 |
| 리포트에 `🔤 판단이 허용 어휘를 벗어남` 경고 | 모델이 다른 리포트 유형의 판단 어휘를 가져다 쓴 것입니다(예: 금리 리포트에 "위험선호"). 배너의 색과 아이콘을 믿지 마세요 |
| 야간선물 전일 종가가 같은 날 바뀜 | 전일 종가를 스크래핑한 등락률로 **역산**하기 때문입니다(`price / (1 + pct/100)`). 페이지가 기준선을 바꾸면 값이 움직입니다. 이제 카드와 원본 데이터에 `(등락률 역산)`으로 표시합니다 |
